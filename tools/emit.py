 ...
 *
 * A traced Cybiko image as C: one label per instruction, direct transfers as
 * `goto`, and a dense dispatch switch for everything computed.
 */
...
def emit(d, ops, reached, out):
    ...
    for k, a in enumerate(addrs):
        ...
        if i.kind in (K_TRAP, K_UNK):
            w("    c->trapped = 1; c->pc = 0x%06XU; return;\n" % a)
            skipped += 1
            continue
        ...
        if k + 1 < len(addrs) and addrs[k + 1] != nxt:
            w("    c->pc = 0x%06XU; goto dispatch;\n" % nxt)
